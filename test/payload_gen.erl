...
-type binary_payload() :: { binary()
                          , _ChunkNum :: non_neg_integer()
                          , _ChunkCnt :: non_neg_integer()
                          }.

%% For performance reasons we treat regular lists as streams, see `next/1'
-opaque cont(Data) :: fun(() -> stream(Data))
                    | stream(Data).

-type stream(Data) :: maybe_improper_list(Data, cont(Data))
                    | ?end_of_stream.
...
binary_stream_gen(ChunkSize) when ChunkSize rem ?hash_size =:= 0 ->
  ?LET({Seed, Size}, {nat(), range(1, 16#100000)},
...
interleaved_streams_gen(MaxNStreams, StreamType) ->
  ?LET(NStreams, range(1, MaxNStreams),
       ?LET(Streams, vector(NStreams, StreamType),
            begin
...
-spec interleave_streams( [{Tag, stream(Data)}]
                        ) -> stream({Tag, Data}).
...
interleave_chunks(Streams0) ->
  Streams = [{Tag, generate_chunk(Payload, ChunkSize)}
...
-spec interleave_chunks( [payload()]
                       , non_neg_integer()
                       ) -> tagged_binstream().
interleave_chunks(Streams0, ChunkSize) ->
  Streams = [{Seed, generate_chunk({Seed, Size}, ChunkSize)}
             || {Seed, Size} <- Streams0],
  interleave_streams(Streams).

%% @doc Generate chunks of data and feed them into
%% `Callback'
-spec generate_chunks( payload()
                     , integer()
                     , fun((binary()) -> A)
                     ) -> [A].
...
-spec check_consistency( payload()
                       , integer()
...
check_consistency({Seed, Size}, SampleSize, Callback) ->
  ...
  lists:foreach( fun(N) when N < Size ->
                     Expected = do_get_byte(N, SeedHash),
                     ?assertEqual( {N, {ok, Expected}}
                                 , {N, Callback(N)}
                                 );
                    (N) ->
...
-spec check_file_consistency( payload()
                            , integer()
                            , file:filename()
                            ) -> ok.
...
generate_chunk(State0 = #chunk_state{offset = Offset, chunk_size = ChunkSize}) ->
  ...
  Payload = generate_chunk( State#chunk_state.seed
                          , Offset
                          , ChunkSize
...
generate_chunk(Seed, Offset, ChunkSize, Size) ->
  ...
  Payload  = iolist_to_binary([generator_fun(I, SeedHash)
